classdef GenericTrimSolver < handle
    ...
    methods
        ...
        function [x_trim, u_trim, converged, info] = solve_trim(obj, altitude, velocity, gamma, phi, turn_rate)
            ...
            ac = obj.aircraft;
            if isempty(ac) || ~isvalid(ac), error('GenericTrimSolver:InvalidAircraft','aircraft is empty or invalid'); end
            if isempty(altitude) || isempty(velocity), error('GenericTrimSolver:InvalidInputs','altitude and velocity must be provided'); end

            n_cs = numel(ac.control_surfaces);
            n_pe = numel(ac.propulsive_elements);

            if n_cs == 0, error('GenericTrimSolver:NoControls','No control surfaces registered'); end
            ...
            if isempty(pitch_idx), error('GenericTrimSolver:NoPitchControl','No pitch-axis control surface found'); end
            ...
            if is_symmetric
                ...
                if isempty(obj.initial_guess)
                    z0 = [deg2rad(3); 0; deg2rad(0); 0; 0; 0.5];
                else
                    z0 = obj.initial_guess(:);
