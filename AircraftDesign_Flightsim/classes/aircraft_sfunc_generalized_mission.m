function aircraft_sfunc_generalized_mission(block)
setup(block);
end
...
if ~isempty(autopilot) && isobject(autopilot) && isprop(autopilot,'enabled') && isprop(autopilot,'mode')
    ...
    if autopilot.enabled && autopilot.mode ~= "off"
        if evalin('base','exist(''mission'',''var'')')
            ...
            % Get reference pitch from mission profile
            theta_ref = interp1(mission.time_vector, mission.state_profile(8,:), t_m, 'linear', 'extrap');
            
            % Set target for pitch autopilot
            if isprop(autopilot, 'theta_target')
                autopilot.theta_target = theta_ref;
            end
        end

        if ~ap_prev_enabled
            try
                if ismethod(autopilot, 'initialize_from_trim')
                    autopilot.initialize_from_trim(x, u_in);
                end
            catch
            end
        end
        ...
        try
            ...
            warning('Autopilot error: %s', ME.message);
            u_cmd = u_in;
        end
    end
end
