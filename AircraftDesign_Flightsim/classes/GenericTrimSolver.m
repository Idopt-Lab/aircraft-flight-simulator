        ...
        function [x_trim, u_trim, converged, info] = solve_trim(obj, altitude, velocity, gamma, phi, turn_rate)
            ...
            if obj.use_fmincon
                opts = optimoptions('fmincon','Display','off','MaxIterations',obj.max_iterations, ...
                    'MaxFunctionEvaluations',100000,'OptimalityTolerance',1e-16,'StepTolerance',1e-16, ...
                ...
                try
                    ...
                    warning('fmincon failed: %s. Trying fminsearch', ME.message);
                    try
                        opts_fms = optimset('Display','off','MaxIter',10000,'MaxFunEvals',50000,'TolFun',1e-18,'TolX',1e-16);
                        z_star = fminsearch(cost, z0, opts_fms);
                    catch
                        z_star = z0;
                    end
                end
                ...
                if isempty(obj.fminsearch_options)
                    opts = optimset('Display','off','MaxIter',obj.max_iterations,'MaxFunEvals',100000,'TolFun',1e-18,'TolX',1e-16);
                else
