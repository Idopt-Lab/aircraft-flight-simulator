classdef MissionPlanner < handle
    ...
    methods
        ...
        function compute_trim_schedule(obj, varargin)
            ...
            for si = 1:numel(seg_ids)
                ...
                if is_level
                    ...
                    if ~isempty(x_last), obj.seed_solver(solver, x_last, u_last, n_cs, n_total); end
                    ...
                    if conv, x_last = xt; u_last = ut; end

                    fprintf('[%s] h=%.0fm M=%.3f → %s\n', char(typek), h, Mk, obj.conv_str(conv,ut,xt,n_cs));

                else
               
                    h0 = hseg(1);  h1 = hseg(end);
                    dh = p.Results.dh_m;
        ...
        function plot_mission(obj)
            ...
            subplot(3,1,1); plot(t, alt,'b','LineWidth',2); grid on
            ylabel('Altitude (km)'); title('Mission Profile')
            subplot(3,1,2); plot(t, obj.mission.mach_profile,'r','LineWidth',2); grid on
            ylabel('Mach')
            subplot(3,1,3); plot(t, gam,'m','LineWidth',2); grid on
    ...
    methods (Access = private)
        ...
        function blend_boundaries(obj, frac)
            % cosine blend controls across each segment boundary
            ...
            for k = 1:numel(bounds)
                ...
                ramp_a = idxs_a(1:n_a);
                t0 = tvec(ramp_a(1)); t1 = tvec(ramp_a(end));
                for idx = ramp_a(:)'
                    w = 0.5*(1 - cos(pi*(tvec(idx)-t0)/max(t1-t0,1e-9)));
                    u(:,idx) = (1-w)*u_bef + w*u_aft;
                end
        ...
        function seed_solver(~, solver, x_last, u_last, n_cs, n_total)
            ...
            alpha0 = atan2(x_last(6), max(x_last(4), 1e-6));
            thr0   = 0.7;
            if n_total > n_cs, thr0 = u_last(n_cs+1); end
          
            alpha0 = max(-0.1, min(deg2rad(15), alpha0));
            thr0   = max(0.1,  min(1.0, thr0));
            solver.initial_guess = [alpha0; 0; thr0];
        end

        function [x,u,conv] = trim_point(~, solver, h, Vk, Mk, gamma, typek, n_cs, n_total)
         
            has_warm = ~isempty(solver.initial_guess);

            if ~has_warm
                if abs(gamma) < deg2rad(0.5)
                    if Mk < 0.4,      solver.initial_guess = [deg2rad(4);   0; 0.55];
                    elseif Mk < 0.65, solver.initial_guess = [deg2rad(2.5); 0; 0.70];
                    else,             solver.initial_guess = [deg2rad(1.5); 0; 0.85];
                    end
                elseif gamma > 0
                  
                    if Mk < 0.5, solver.initial_guess = [deg2rad(3);  deg2rad(-1); 0.90];
                    else,        solver.initial_guess = [deg2rad(2);  deg2rad(-1); 0.85];
                    end
                else
                    if Mk < 0.4, solver.initial_guess = [deg2rad(3); deg2rad(2); 0.45];
                    else,        solver.initial_guess = [deg2rad(2); deg2rad(1.5); 0.60];
                    end
                end
            end
            ...
            if ~conv
                ...
                guesses = {[deg2rad(2);  deg2rad(-1); 0.92], ...
                           [deg2rad(4);  deg2rad(-2); 0.88], ...
                           [deg2rad(6);  deg2rad(-3); 0.93], ...
                           [deg2rad(1);  0;           0.80], ...
                           [deg2rad(8);  deg2rad(-4); 0.95], ...
                           [deg2rad(10); deg2rad(-5); 0.98]};
            ...
            if ~conv
                ...
                x(6) = Vk*sin(gamma);
                u    = zeros(n_total,1);
                if n_total > n_cs, u(n_cs+1:end) = 0.85; end
            end
        end

        function s = conv_str(~, conv, u, x, n_cs)
            if conv
                thr   = u(n_cs+1);
                alpha = rad2deg(atan2(x(6), x(4)));
                s = sprintf('OK  α=%.2f°  thr=%.3f', alpha, thr);
            else
                s = 'FAILED';
            end
        end
        ...
        function [T,H,V,G,M,seg,t] = append_samples(~,T,H,V,G,M,seg,tv,hv,vv,gv,mv,id)
            if isempty(T)
                ...
                seg=id*ones(numel(tv),1); t=T(end); return
            end
            if tv(1) <= T(end), tv = tv + (T(end)-tv(1)+1e-6); end
            T=[T;tv(:)]; H=[H;hv(:)]; V=[V;vv(:)];
            G=[G;gv(:)]; M=[M;mv(:)];
            seg=[seg; id*ones(numel(tv),1)]; t=T(end);
        end
        ...
        function [tv,hv,vv,gv,mv] = segment_climb_descent(obj,t0,h0,h1,mach,mach_end,gamma,gamma_end)
            if nargin < 7, gamma = 0; end
            if nargin < 8, gamma_end = gamma; end
            dh = h1 - h0;
            if abs(dh) < 1e-9
            ...
            hdot(abs(hdot) < 1e-3) = sign(dh)*5.0;
            Tseg = max(sum(abs(diff(h_sub))./abs(hdot(1:end-1))), obj.dt);
