...
function data = parse_datcom(filepath)
    ...
    while i <= numel(lines)
        ...
        if contains(line,'ALPHA') && contains(line,'CD') && contains(line,'CL') && contains(line,'CM')
            ...
            while k <= numel(lines)
                ...
                if ~isempty(vals) && numel(vals) >= 4
                    ...
                    if abs(alpha) < 30
                        ...
                        if numel(vals) >= 8  && abs(alpha) <= 2.5 && abs(vals(8))  > 0.01  && isnan(temp_CLA), temp_CLA = vals(8); end
                        if numel(vals) >= 9  && abs(alpha) <= 2.5 && abs(vals(9))  > 0.01  && isnan(temp_CMA), temp_CMA = vals(9); end
                        if numel(vals) >= 10 && abs(alpha) <= 2.5 && abs(vals(10)) > 0.001 && isnan(temp_CYB), temp_CYB = vals(10); end
                        if numel(vals) >= 11 && abs(alpha) <= 2.5 && abs(vals(11)) > 0.001 && isnan(temp_CNB), temp_CNB = vals(11); end
            ...
            if ~isempty(temp_alpha) && numel(temp_alpha) > numel(alpha_vec)
...
function C = eval_datcom(x, u, geom, data)
    ...
    if V>1
        ...
        CL = CL + data.CLQ*q;
        Cm = Cm + data.CMQ*q;
        Cl = Cl + data.CLP*p + data.CLR*r;
        Cn = Cn + data.CNP*p + data.CNR*r;
        CY = CY + data.CYP*p;
    end
