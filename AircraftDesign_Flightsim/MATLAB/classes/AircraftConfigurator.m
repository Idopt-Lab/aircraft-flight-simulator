classdef AircraftConfigurator < handle
    ...
    methods
        ...
        function obj = AircraftConfigurator(ac)
            obj.aircraft = ac;
            obj.aircraft.ensure_gravity_source();
        end
        ...
        function comp = add_component(obj, varargin)
            ...
            if ~obj.aircraft.has_frame(frame_name)
                obj.aircraft.add_frame(frame_name, s.parent_frame, s.position(:), s.dcm_fn);
            else
            ...
            comp = Component(s.name, s.mass, s.cg_local, s.inertia, comp_frame, s.type);
        ...
        function add_control_surface(obj, varargin)
            ...
            addParameter(p, 'dCL_force', 0);
            addParameter(p, 'dCD_force', 0);
            addParameter(p, 'dCY_force', 0);
            parse(p, varargin{:});
            s = p.Results;

            cs = ControlSurface(s.name, s.surface_type, s.classification, s.axis, s.max_deflection, s.min_deflection, s.dCl, s.dCm, s.dCn, s.dCL_force, s.dCD_force, s.dCY_force);
        ...
        function [pe, comp] = add_propulsive_element(obj, varargin)
            ...
            addParameter(p, 'engine_model', []);
            addParameter(p, 'propeller_model', []);
            addParameter(p, 'rotation_direction', 1);
            ...
            if ~obj.aircraft.has_frame(frame_name)
                obj.aircraft.add_frame(frame_name, s.parent_frame, s.position(:), @(x) obj.mount_dcm_from_euler(s.mount_euler));
            else
                obj.aircraft.update_frame_position(frame_name, s.position(:));
                obj.aircraft.update_frame_orientation(frame_name, @(x) obj.mount_dcm_from_euler(s.mount_euler));
            end
            ...
            switch et
                case {'turbojet','turbofan','turbofan_afterburning'}
                    pe = TurbofanPropulsion(s.name, thrust_frame, dir, s.max_output, s.fuel_rate, s.thrust_model);

                case {'afterburning_turbojet'}
                    pe = AfterburningTurbojet(s.name, thrust_frame, dir);

                case {'cfm56','cfm56_turbofan'}
                    pe = CFM56Turbofan(s.name, thrust_frame, dir);

                case {'propeller','prop'}
                    pe = PropellerPropulsion(s.name, thrust_frame, dir, s.max_output, s.fuel_rate, s.thrust_model);

                    has_engine_model = ~isempty(s.engine_model);
                    has_propeller_model = ~isempty(s.propeller_model);
                    if xor(has_engine_model,has_propeller_model)
                        error('AircraftConfigurator:IncompletePropellerModel', ['Provide both engine_model and propeller_model ', 'for a coupled propeller installation.']);
                    end
                    if isempty(s.thrust_model) && ~has_engine_model
                        error('AircraftConfigurator:PropellerModelsRequired', ['A physical propeller installation requires ', 'engine_model and propeller_model.']);
                    end
                    if has_engine_model
                        pe.set_models(s.engine_model,s.propeller_model);
                    end
                    pe.set_rotation_direction(s.rotation_direction);
                    pe.set_air_velocity_frame(obj.aircraft.get_frame(obj.aircraft.body_frame_name));

                case {'rotor','rotor_variable','multirotor','rotor_blockset'}
                    pe = RotorPropulsion(s.name, thrust_frame, dir, s.thrust_model);

                case {'electric'}
                    pe = ElectricPropulsion(s.name, thrust_frame, dir, s.max_output, s.thrust_model);

                otherwise
                    pe = CustomPropulsion(s.name, thrust_frame, dir, s.thrust_model);
            end

            obj.aircraft.add_propulsive_element(pe);

            comp = Component(s.name, s.mass, s.cg_local, s.inertia, thrust_frame, "engine");
        ...
        function solver = add_aero_solver(obj, aero_model, varargin)
            ...
            addParameter(p, 'position', obj.aircraft.geometry.get_reference_point());
            addParameter(p, 'dcm_fn', @(x) eye(3));
            ...
            reference_point = s.geom.get_reference_point();
            position = s.position(:);
            if numel(position) ~= 3 || any(~isfinite(position)) || norm(position-reference_point) > 1e-9
                error('AircraftConfigurator:AeroReferencePointMismatch', ['Aerodynamic frame position must equal the geometry ', 'reference point.']);
            end
            if ~strcmp(char(string(s.parent_frame)), char(obj.aircraft.body_frame_name))
                error('AircraftConfigurator:AeroParentFrameMismatch', 'Aerodynamic frame parent must be the body frame.');
            end
            C_test = s.dcm_fn(zeros(12,1));
            if ~isequal(size(C_test),[3 3]) || any(~isfinite(C_test(:))) || norm(C_test-eye(3),'fro') > 1e-9
                error('AircraftConfigurator:AeroAxesMismatch', 'Aerodynamic frame must be body-axis aligned.');
            end
            ...
            if ~obj.aircraft.has_frame(frame_name)
                obj.aircraft.add_frame(frame_name, s.parent_frame, position, s.dcm_fn);
            else
                obj.aircraft.update_frame_position(frame_name, position);
                obj.aircraft.update_frame_orientation(frame_name, s.dcm_fn);
            end

            aero_frame = obj.aircraft.get_frame(frame_name);

            solver = AeroLoadSolver(aero_model, s.geom, obj.aircraft, aero_frame);
        ...
        function print_configuration(obj)
            ...
            for i = 1:numel(ac.control_surfaces)
                ...
                fprintf('  %d) %-12s type=%-10s class=%-10s axis=[%.0f %.0f %.0f]\n', i, char(string(cs.name)), char(string(cs.surface_type)), char(string(cs.classification)), cs.axis(1), cs.axis(2), cs.axis(3));
            end
            ...
            for k = 1:numel(ac.propulsive_elements)
                ...
                fprintf('  %d) %-15s class=%-25s frame=%-20s axis=[%.3f %.3f %.3f]\n', k, char(pe_name), pe_class, char(pe_frame), pe_axis(1), pe_axis(2), pe_axis(3));
            end
